...
from training.custom_callbacks import ADJUSTLR, MulticlassAUC
...
class DTFT:
    def __init__(self,args):
        ...
        slide_pseudo_feat,slide_sub_preds = GROUP_MIL(distill="AFS")(
                                            [self.inputs['bag'],
                                           self.inputs['group_1'],
                                           self.inputs['group_2'],
                                           self.inputs['group_3'],
        ...
        gSlidePred = Attention_with_Classifier(L=128)(self.inputs['att_tensor'])
    ...
    def train(self, train_bags, val_bags, fold):
        ...
        val_gen = Alternating_task_generator(batch_size=1, csv_file=self.csv_file, feature_path=self.feature_path,
                                             filenames=val_bags, train=False)
        ...
        reduce_rl_plateau_1 = ADJUSTLR(factor=0.2,
                                             freq=100,
                                             verbose=1,
                                             optim_lr=optimizer_1.learning_rate,
                                             mode="min")

        reduce_rl_plateau_2 = ADJUSTLR(factor=0.2,
                                             freq=100,
                                             verbose=1,
                                             optim_lr=optimizer_2.learning_rate,
                                             mode="min")
        ...
        val_auc_1_metric =MulticlassAUC(pos_label=1)
        val_auc_2_metric = MulticlassAUC(pos_label=1)
        ...
        def val_step_1(x, y):
            ...
            val_auc_1_metric.update_state(y,Y_prob_1)
        ...
        def val_step_2(x, y):
            ...
            val_auc_2_metric.update_state(y, Y_prob_2)
            return {"val_loss_2": val_loss_2_tracker.result()}

        reduce_rl_plateau_1.on_train_begin()
        reduce_rl_plateau_2.on_train_begin()
        for epoch in range(self.epochs):
            ...
            val_auc_1_metric.reset_states()
            val_auc_2_metric.reset_states()
            ...
            logs["val_auc_1"] = val_auc_1_metric.result()
            logs["val_auc_2"] = val_auc_2_metric.result()

            val_acc_1 = val_acc_1_metric.result()
            val_acc_2 = val_acc_2_metric.result()

            val_auc_1=val_auc_1_metric.result()
            val_auc_2= val_auc_2_metric.result()

            print("Validation acc 1: %.4f" % (float(val_acc_1),))
            print("Validation acc 2: %.4f" % (float(val_acc_2),))

            print("Validation AUC 1: %.4f" % (float(val_auc_1),))
            print("Validation AUC 2: %.4f" % (float(val_auc_2),))

            print("Time taken: %.2fs" % (time.time() - start_time))

            reduce_rl_plateau_2.on_epoch_end(epoch)
            reduce_rl_plateau_1.on_epoch_end(epoch)
    ...
    def predict(self, test_bags, fold, tier_1, tier_2):
        ...
        pos_label=1
        # test_auc_metric = MulticlassAUC(pos_label=1)
        ...
        def test_step_1(images, labels):
            ...
            # test_auc_metric.update_state (labels, Y_prob_2)
            # test_acc_metric.update_state(labels, pred_2)
        ...
        for enum, (x_batch_val, y_batch_val) in enumerate(test_gen):
                ...
                y_true.append(y_batch_val.numpy().tolist()[0])
        ...
        accuracy, precision, recall, F1, auc_score = eval_metric(y_score[:, pos_label], y_true)

        print("AUC {}".format(auc_score))

        print("Test acc 2: %.4f" % (float(accuracy),))
        ...
        return accuracy, auc_score, precision, recall, fscore