...
parser.add_argument('--checkpoint',
                    default='/home/mvries/Documents/GitHub/DTFD-MIL/debug_log/camelyon_best_model.pth',
                    type=str)
...
def main():
    ...
    model1 = Model1(params, params.checkpoint)
    ...
    trainable_parameters += list(model1.parameters())
    ...
    if params.task != 'evaluate':
        for ii in range(params.EPOCH):
            ...
            train_attention_preFeature_DTFD(model1=model1,
                                            UClassifier=attCls,
            ...
            auc_val = test_attention_DTFD_preFeat_MultipleMean(model1=model1,
                                                               UClassifier=attCls,
            ...
            tauc = test_attention_DTFD_preFeat_MultipleMean(model1=model1,
                                                            UClassifier=attCls,
            ...
            if ii > int(params.EPOCH * 0.8):
                if auc_val > best_auc:
                    ...
                    if params.isSaveModel:
                        tsave_dict = {
                            'classifier': model1.classifier.state_dict(),
                            'dim_reduction': model1.dimReduction.state_dict(),
                            'attention': model1.attention.state_dict(),
        ...
        save_path = '/home/mvries/Documents/GitHub/DTFD-MIL/debug_log/camelyon_best_model.pth'
        checkpoint = torch.load(save_path, map_location='cuda')
        model1.load_from_checkpoint(checkpoint)
        auc_eval = evaluate_trained_model(model1=model1,
                                   UClassifier=attCls,
                                   mDATA_list=(SlideNames_test, FeatList_test, Label_test),
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, model1, UClassifier, epoch,
...
def evaluate_trained_model(mDATA_list,
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                for jj in range(params.num_MeanInference):
                    ...
                    for tindex in index_chunk_list:
                        ...
                        all_pred_logits.append(patch_pred_logits)
    ...
    return auc_1, gPred_0, gPred_1, gt_0, gt_1, all_pred_logits
