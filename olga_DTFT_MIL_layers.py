...
class MILAttentionLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, isNorm=True, training=None):
...
class Attention_with_Classifier(tf.keras.layers.Layer):
    ...
    def __init__(self, L=128, num_cls=2, droprate=0, **kwargs):
        self.attention = MILAttentionLayer(weight_params_dim=L, use_gated=True)
...
class GROUP_MIL(tf.keras.layers.Layer):
    ...
    def __init__(self,distill, **kwargs):
        ...
        self.attention = MILAttentionLayer(128,use_gated=True, name="MILAttentionLayer")
    ...
    def  call(self, input_tensor, training):
        ...
        if not training:
            ...
            AA = self.attention(midFeat, isNorm=False)
        ...
        for enum, tindex in enumerate(range(len(index_chunk_list))):
                ...
                if training:
                    ...
                    tmidFeat = tf.squeeze(tf.gather(midFeat, tf.cast(tindex,tf.int32)))
                    tmidFeat = tf.ensure_shape(tmidFeat, [None, 512])
